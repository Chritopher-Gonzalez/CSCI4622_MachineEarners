...
class Deck:
    def __init__(self, numDecks=1, removeSubset=False):
        self.deck = []  # start with an empty list#
        self.numOfEachCard = np.zeros(10)
        ...
        for i in range(numDecks):
            for suit in suits:
                for rank in ranks:
                    ...
                    self.numOfEachCard[values[rank] - 2] += 1

        self.shuffle()

        if(removeSubset):
            numCardsToRemove = random.randint(0, len(self.deck) - 20)
            # assuming worst case where only 2s are drawn, 20 cards are required to complete a game with
            # 1 player vs 1 dealer

            while(numCardsToRemove > 0):
                card = self.deck.pop()
                self.numOfEachCard[values[card.rank] - 2] -= 1
                numCardsToRemove -= 1
    ...
    def getDeckComposition(self):
        return self.numOfEachCard / np.sum(self.numOfEachCard)
...
class Game:
    def __init__(self, removeSubset=False):
        # Create & shuffle the deck
        self.playing = True

        if(removeSubset):
            self.deck = Deck(removeSubset=True)
        else:
            self.deck = Deck()
        #self.deck.shuffle() // deck is shuffled at initialization
        ...
        self.deckComposition = self.deck.getDeckComposition()
    ...
    def getData(self):
        ...
        data1 = np.array([self.action, self.isSoft, self.playerValue, self.shownCardIsAce, self.shownDealerCard])
